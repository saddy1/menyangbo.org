  ...
  <div class="topbar-wrap flex items-center gap-3 px-4 py-3 border-b bg-white sticky top-0 z-30">
    ...
    <div class="searchBox relative w-[360px] ml-auto">
      ...
      <div x-show="searchOpen" x-cloak class="absolute mt-1 w-full bg-white border rounded-lg shadow-lg max-h-72 overflow-auto">
        ...
        <div x-show="results.length===0" class="px-3 py-2 text-sm text-slate-500">
          No results
        </div>
      </div>
    </div>
  ...
  <div x-show="modalOpen" x-cloak class="fixed inset-0 z-50">
    ...
    <div class="absolute right-0 top-0 h-full w-full sm:w-[420px] bg-white shadow-xl p-5 overflow-y-auto">
      ...
      <template x-if="person">
        <div class="space-y-4">
          <div class="flex items-start gap-3 p-4 border rounded-xl">
          ...
          <div class="p-4 border rounded-xl">
            ...
            <template x-for="pp in (person.parents||[])" :key="pp.id">
              <button class="block w-full text-left px-3 py-2 rounded-lg hover:bg-slate-50"
                      @click="jumpTo(pp.id)">
...
toENdigits(s){
  return String(s||'').replace(/[०-९]/g, d => this.NE2EN[d] || d);
    ...
    genderColor(g){
      if(g === "male") return "#2563eb";     // blue
      if(g === "female") return "#ef4444";   // red
    ...
    async reload(){
      ...
      // set top bar pusta from root node
    ...
    draw(data){
      ...
      const layout = d3.tree().nodeSize([85, 145]);
      layout(root);
      ...
      node.append("use")
        .attr("href", d => {
          if(d.data.gender === "male") return "#icon-male";
          if(d.data.gender === "female") return "#icon-female";
          return "#icon-unk";
        })
    ...
    selectResult(r){
      ...
      this.currentPusta = r.pusta || this.currentPusta; // show pusta on top immediately
      this.reload();
    },
