...
<div class="min-h-[70vh] flex items-center justify-center py-8">
  <div class="w-full max-w-md">
    <div class="bg-white rounded-2xl shadow-lg border border-slate-200 p-8 text-center">
      ...
      <h1 class="text-xl font-bold text-slate-800 mb-2">इमेल प्रमाणीकरण गर्नुहोस्</h1>
      <p class="text-sm text-slate-600 mb-6">
        हामीले तपाईंको इमेल <strong>{{ auth()->user()->email }}</strong> मा एउटा प्रमाणीकरण लिङ्क पठाएका छौं।
      ...
      <form method="POST" action="{{ route('verification.resend') }}">
        ...
        <button type="submit"
          ...
          पुनः इमेल पठाउनुहोस्
        </button>
      </form>

      <form method="POST" action="{{ route('logout') }}">
        @csrf
        <button type="submit" class="text-sm text-slate-500 hover:text-slate-700 underline">लगआउट</button>
      </form>
    </div>
  </div>
</div>
